/* =============================
   RCG CONFIGURATOR — SINGLE FILE (CLEAN / STABLE)
...
(() => {
  ...
  function maybeShowEdgeCallout() {
    ...
    window.setTimeout(() => callout.classList.add('rcg-hidden'), 2400);
    ...
    if (state.shape === 'rectangle' && state.sinks.length) {
      ...
      state.sinks.forEach((snk, idx) => {
        ...
        node.addEventListener('pointerdown', (e) => {
          ...
          activeSinkDrag = { idx, tpl, bbox, s, ox, oy };
  ...
  function computePricing(zip) {
    ...
    const sinks = (state.shape === 'rectangle')
      ? state.sinks.reduce((acc, s) => acc + (SINK_PRICES[s.key] || 0), 0)
      : 0;
